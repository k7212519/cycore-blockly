...
const { startBleDebuggerServer } = require('./server');
...
function startRpcServer() {
  ...
  try {
    core = createBleDebuggerCore({ sendEvent });
  } catch (error) {
...
async function startServeMode(rawArgs) {
  ...
  try {
    serverHandle = await startBleDebuggerServer({
...
if (requestedCommand === 'rpc' || requestedCommand === 'server') {
  ...
  void runCli(requestedCommand, cliArgs.slice(1), {
    createCore: () => createBleDebuggerCore(),
