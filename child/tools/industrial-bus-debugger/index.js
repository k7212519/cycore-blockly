...
const { startIndustrialBusDebuggerServer } = require('./server');
...
function startRpcServer() {
  ...
  try {
    core = createIndustrialBusDebuggerCore({ sendEvent });
  } catch (error) {
...
async function startServeMode(rawArgs) {
  ...
  try {
    serverHandle = await startIndustrialBusDebuggerServer({
...
if (requestedCommand === 'rpc' || requestedCommand === 'server') {
  ...
  void runCli(requestedCommand, cliArgs.slice(1), {
    createCore: () => createIndustrialBusDebuggerCore(),
