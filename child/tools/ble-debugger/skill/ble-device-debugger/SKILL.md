...
description: "Use when developing BLE firmware or debugging Bluetooth Low Energy devices in Aily Blockly or Aily Chat, including GATT service design, UUID planning, BLE scanning, connecting, reading, writing, notifications, payload formats, adapter state issues, and the child/tools/ble-debugger tool."
metadata:
  version: "1.0.0"
  author: aily-team
  scope: global
  agents: mainAgent
  auto-activate: false
  tags: ble,bluetooth,gatt,device-debugging,esp32,aily-blockly,child-tool
---

# BLE Device Debugger

Use this skill to help develop BLE device behavior and debug BLE communication with Aily's BLE Debugger child tool.
...
5. Ask for the exact symptom only when it changes the debugging path, such as not visible in scan, visible but cannot connect, GATT missing, read/write failing, or notifications silent.
...
Use the existing child tool boundaries instead of moving logic across layers:

- `child/tools/index.json` registers `ble-debugger` as a child tool with route `/child-tool/ble-debugger`.
- `src/app/services/child-tool-process.service.ts` starts the backend with `node index.js serve --host 127.0.0.1 --port 0`.
- `src/app/tools/child-tool-host/child-tool-host.component.ts` embeds the child UI, passes language/theme via query params, and exchanges host context through Penpal.
...
- `child/tools/ble-debugger/core.js` owns the BLE implementation through `@abandonware/noble`: adapter state, scanning, connection, GATT discovery, reads, writes, subscriptions, and cleanup.
...
Interpret CLI output as JSON. If hardware is unavailable, use `--help` and code-level verification instead of inventing scan results.
...
- Use one source of truth for UUID strings. Avoid mixing 16-bit, 32-bit, dashed 128-bit, and compact 128-bit forms unless normalized.
...
- Document the test matrix in the answer: scan result, GATT map, read value, write payload, notification event, and expected device-side log.

## Modifying The Tool

When changing the BLE Debugger tool itself:

- Put BLE transport and adapter fixes in `core.js`.
- Put server, static serving, token, and WebSocket RPC fixes in `server.js`.
...
- Keep the JSON contract stable unless the UI, CLI, and host are updated together.
