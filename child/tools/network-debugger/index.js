...
const { startNetworkDebuggerServer } = require('./server');
...
function startRpcServer() {
  ...
  try {
    core = createNetworkDebuggerCore({ sendEvent });
  } catch (error) {
...
async function startServeMode(rawArgs) {
  ...
  try {
    serverHandle = await startNetworkDebuggerServer({
...
if (requestedCommand === 'rpc' || requestedCommand === 'server') {
  ...
  void runCli(requestedCommand, cliArgs.slice(1), {
    createCore: () => createNetworkDebuggerCore(),
